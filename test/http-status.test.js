...
const assertErrorShape = (res, status, context) => {
  ...
  assert.equal(
    ...
    'string',
    `${context} must carry a string error (${res.raw})`
  );
...
const probeApp = () => {
  ...
  probe.get('/bad-request-details', (req, res, next) =>
    next(
      badRequest('Validation failed', [{ location: 'body', field: 'a', message: 'm', type: 't' }])
    )
  );
  ...
  probe.get('/unclassified', (req, res, next) => {
    const err = new Error(
      "ENOENT: no such file or directory, open '/home/secret/data/devops.json'"
    );
    err.code = 'ENOENT';
...
test('a path whose percent-encoding cannot be decoded answers 400, not 500', async () => {
  ...
  for (const routePath of ['/api/models/%E0%A4%A', '/api/models/%', '/api/models/%zz.json']) {
...
test('every router renders its failures through the shared handler', async () => {
  ...
  for (const [name, router] of routers) {
    ...
    const attached = last && !last.route && last.handle && last.handle.name === 'errorHandler';
    if (!attached) missing.push(name);
  }
  assert.deepEqual(missing, [], 'these routers do not end with the shared error handler');
});
...
test('reading a data recorder that does not exist answers 404', async () => {
  const res = await request(server, 'GET', `/api/data-recorders/models/${unique('absent')}.json`);
...
test('a run is recognised as running by the state the simulation actually keeps', () => {
  ...
  assert.equal(simulation.isRunning, undefined, 'a guard on isRunning would never fire');
