...
test('bodies with mistyped fields are rejected', async () => {
  const cases = [
    ...
    [
      'POST',
      '/api/data-storage',
      { dataStorage: { protocol: 'REDIS', connConfig: { host: 'h', port: 1 } } },
    ],
    [
      'POST',
      '/api/data-storage',
      { dataStorage: { protocol: 'MONGODB', connConfig: { host: 'h', port: 99999 } } },
    ],
...
test('query parameters declared as strings cannot arrive as arrays', async () => {
  for (const routePath of [
    '/api/events?datasetId=a&datasetId=b',
    '/api/reports?testCampaignId=a&testCampaignId=b',
  ]) {
    const res = await request(server, 'GET', routePath);
    assertValidationError(res, `GET ${routePath}`);
  }
...
test('the data recorder lifecycle the dashboard drives still succeeds', async () => {
  ...
  try {
    ...
    await request(server, 'DELETE', `/api/data-recorders/models/${encodeURIComponent(fileName)}`);
    assert.ok(
...
test('the database-backed payloads the dashboard sends still satisfy their schemas', async () => {
  ...
  const cases = [
    ...
    [
      ...
      {
        body: {
          dataset: { id: 'ds-1', name: 'A data set', tags: [], source: 'RECORDED', ...persisted },
        },
      },
    ],
    [
    ...
    [
      ...
      'GET',
      '/',
      { query: { datasetId: 'ds-1', startTime: '0', endTime: '9999999999', page: '0' } },
    ],
    [
      reportRouter,
      'GET',
      '/',
      {
        query: { topologyFileName: '202402-Temperature-Controller.json', testCampaignId: 'tcp-1' },
      },
    ],
...
const _startRun = (body) => validateOnly(simulationRouter, 'POST', '/start', { body });
...
test('the devops configuration cannot carry a database connection of any shape', async () => {
  for (const connConfig of hostileConnConfigs) {
    ...
    assertValidationError(
      res,
      `POST /api/devops with host ${JSON.stringify(connConfig.host.slice(0, 40))}`
    );
    assert.ok(
    ...
    [
      {
        dataStorage: {
          protocol: 'MONGODB',
          connConfig: { host: 'evil.example.com/tas?x=1', port: 27017 },
        },
      },
      'model.dataStorage.connConfig.host',
    ],
...
test('an event value is accepted in every form the product records', async () => {
  ...
  for (const value of values) {
    ...
    assert.equal(
      outcome.passed,
      true,
      `editing an event whose values are ${JSON.stringify(value)} must succeed: ${JSON.stringify(
        outcome.body
      )}`
    );
  }
...
test('a topology carrying null replay bounds is still accepted', async () => {
  ...
  await assertBodyAccepted(
    ...
    '/',
    { model },
    'a stored topology with null replay bounds'
  );
...
test('a generated log file name round-trips through the log routes', async () => {
  ...
  try {
    ...
    assert.equal(
      removed.body.error,
      null,
      `a generated log name must be deletable: ${removed.raw}`
    );
