...
class Simulation {
  constructor(model, options = null, simulationCallbackWhenFinish = null) {
    ...
    this.reportToken = model.reportToken;
    ...
    if (options) {
      ...
      if (options.reportToken) this.reportToken = options.reportToken;
    ...
    if (!this.newDataset) {
      ...
      this.newDataset = {
        id: this.datasetId ? `data-set-${this.datasetId}-${currentTime}`:`data-set-${currentTime}`,
  ...
  deviceCallbackWhenFinish(devId) {
    // console.log(`[${devId}]A device is going to finish the job`);
    for (let index = 0; index < this.allThings.length; index++) {
      const dev = this.allThings[index];
      if (!dev.isOffline() && dev.id !== devId) {
        console.log(`Device ${dev.id} has not finished yet!!!`);
        return;
      }
    }
    // console.log('This is the last device -> Going to stop the simulation');
  ...
  createDevice(devConfig, isFirstDevice = false) {
    const newThing = new Thing(
      ...
      this.report,
      isFirstDevice,
      () => this.deviceCallbackWhenFinish(devConfig.id)
    );
