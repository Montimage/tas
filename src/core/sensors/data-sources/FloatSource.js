    ...
    switch (beha) {
      ...
      case AB_VALUE_OUT_OF_REGULAR_RANGE:
        if (
          ...
          console.error(
            `[FloatSource] Invalid value range/regular range: ${this.min} - ${this.max} | ${this.regularMin} - ${this.regularMax}`
          );
          value = this.value;
        } else {
      ...
      case AB_VALUE_CHANGE_OUT_OF_REGULAR_STEP:
        if (rmin === null || rmax === null || this.step === null || this.step === 0) {
          console.error(
            `[FloatSource] Invalid value range or step: ${rmin} - ${rmax}, ${this.step}`
          );
          value = this.value;
        } else {
